...
const statusCode = (res, status, name) => {
    switch (status) {
        ...
        default:
            ...
            /* Set Repo Hook */
            chrome.storage.local.set({ gitleet_hook: res.full_name },
                () => {
...
const createRepo = (token, name) => {
    ...
    let data = {
        ...
        private: true,
        auto_init: true,
        description: 'Collection of LeetCode questions - Created using [GITLeet](https://github.com/syedareehaquasar/GITLeet).',
    };
  ...
  */
      ...
      if (xhr.readyState === 4) {
        ...
        if (xhr.status === 200) {
          ...
          if (!bool) {
            ...
            chrome.storage.local.set({ gitleet_hook: null }, () => {
  ...
  $('#hook_button').on('click', () => {
    ...
    if (!option()) {
      ...
      /* 
          ...
          */
      chrome.storage.local.get('gitleet_token', (data) => {
        const token = data.gitleet_token;
        if (token === null || token === undefined) {
          ...
          chrome.storage.local.get('gitleet_username', (data2) => {
            const username = data2.gitleet_username;
            if (!username) {
  ...
  chrome.storage.local.get('mode_type', (data) => {
    ...
    if (mode && mode === 'commit') {
      ...
      chrome.storage.local.get('gitleet_token', (data2) => {
        const token = data2.gitleet_token;
        if (token === null || token === undefined) {
          /* Not authorized yet. */
          $('#error').text(
          ...
          /* Get access to repo */
          chrome.storage.local.get('gitleet_hook', (repoName) => {
            const hook = repoName.gitleet_hook;
            if (!hook) {
              /* Not authorized yet. */
              $('#error').text(
