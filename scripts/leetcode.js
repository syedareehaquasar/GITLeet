...
function uploadGit(
    ...
    chrome.storage.local.get('gitleet_token', (t) => {
        const token = t.gitleet_token;
        if (token) {
            chrome.storage.local.get('mode_type', (m) => {
                const mode = m.mode_type;
                if (mode === 'commit') {
                    /* Get hook */
                    chrome.storage.local.get('gitleet_hook', (h) => {
                        const hook = h.gitleet_hook;
                        if (hook) {
...
chrome.storage.local.get('isSync', (data) => {
    ...
    if (!data || !data.isSync) {
        ...
        chrome.storage.local.set({ isSync: true }, (data) => {
            console.log('gitleet Synced to local values');
        });
    } else {
        console.log('gitleet Local storage already synced!');
    }
});